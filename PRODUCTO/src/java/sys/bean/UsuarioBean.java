...
public class UsuarioBean implements Serializable {
    ...
    public void validaSession() {
        try {
            String urlGestionU = "GestionUsuarios";
            String urlParametros = "Parametros";
            FacesContext context = FacesContext.getCurrentInstance();
            Usuario us = (Usuario) context.getExternalContext().getSessionMap().get("ULogueado");
            HttpServletRequest request = (HttpServletRequest) context.getExternalContext().getRequest();
            String url = request.getRequestURI();

            if (us == null || this.Logueado == null) {
                context.getExternalContext().redirect("/PRODUCTO/faces/Error.xhtml");
            } else if((url.contains(urlGestionU) || url.contains(urlParametros))  && (this.Logueado.getIdPerfil().getIdPerfil() != 1)){
